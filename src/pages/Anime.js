...
function Anime() {
    ...
    const api_url = `https://api.gdriveplayer.us/v1/animes/search?title=${title}`;
    ...
    const showMovie = async () => {
        const result1 = await fetch("https://api.gdriveplayer.us/v1/animes/newest");
    ...
    return (
        <div className='transition-all'>
            <div className='bg-stone-300 dark:bg-black h-screen overflow-x-hidden'>
                    ...
                    <div className='grid grid-cols-1 gap-8 p-12 xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 md:pt-24 pt-28 transition-all transform ease-in-out float-none'>
                        {
                            data && data.map((records) => {
                                ...
                                return (
                                    <div className="flex-wrap group shadow-xl hover:shadow-lg shadow-black rounded-3xl bg-blue-900 overflow-hidden hover:scale-110 duration-500 transition-all ease-in-out" key={ids}><p className='absolute group-hover:block hidden mt-32 text-center mx-20 text-black font-extrabold text-lg'>Genre - {records.genre}</p>
                                        <Link to={`/anime/info/${records.id}`}>
